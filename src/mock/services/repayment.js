...
const List = (options) => {
  ...
  for (let i = 1; i < next; i++) {
    ...
    records.push({
      id: tmpKey,
      loanNo: 'DSB' + tmpKey,
      repaymentTime: Mock.mock('@date'),
      repaymentType: repaymentTypes[Mock.mock('@integer(0, 1)')],
      repaymentPrincipal: Mock.mock('@float(10000, 20000, 2, 2)'),
      loanRate: Mock.mock('@float(0, 1, 2, 2)'),
      repaymentStatus: Mock.mock('@integer(0, 2)'),
      repaymentAllAmount: Mock.mock('@float(10000, 20000, 2, 2)')
    })
  }
...
Mock.mock(/\web\/repayment-record/, 'get', List)
Mock.mock(/\/repayment-record-detail/, 'get', loanDetail)
Mock.mock(/\/repayment-record-statistic/, 'get', loanStatisticList)
Mock.mock(/\/repayment-record-user-statistic/, 'get', loanUserStatisticList)
