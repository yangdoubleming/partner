<template>
    <section>
        <div class="authorize-box">
            ...
            <div class="down">
                <div class="down-left" @click="linkTo('/shop/amazonAuthorizeList')">
                    <i class="el-icon-search"></i>
                    <span>查看授权店铺</span>
                </div>
                <div class="down-right">
                    <i class="el-icon-question"></i>
                    <a href="./amazon.pdf" target="_blank">查看授权帮助</a>
                </div>
            </div>
        </div>
        <a-modal
            ...
            @cancel="handleCancel"
            :footer="null">
            <a class="linkWord" href="./amazon.pdf" target="_blank" >查看授权帮助</a>
            <div label="授权站点" prop="marketPlaceId">
                <a-select style="width: 80%;margin:15px auto;" v-model="amazonValidateForm.marketPlaceId" placeholder="请选择授权站点" >
                    <a-select-option v-for="item in station" :value="item.marketPlaceId" :key="item.marketPlaceId" >
                        {{item.amazonShop}}
                    </a-select-option>
                </a-select>
            </div>
            ...
            <div style="margin:15px auto">
                <a-input placeholder="卖家编号"  v-model="amazonValidateForm.sellerIdentifier" />
            </div>
            <div style="margin:15px auto">
                <a-input placeholder="MWS授权令牌" v-model="amazonValidateForm.mwsToken" />
            </div>
...
<script>
    ...
    import { getStation, amazonAuthorize } from '@/api/shop'
    export default {
        data() {
            return {
               modelStatus:false,
                ...
                amazonLogo: config.publicPath + '/icons/amazonLogo.png',
                amazonCenter: true,
                amazonValidateForm: {
                    marketPlaceId: '',
                    shopName:'',
        ...
        methods: {
            ...
            submitAmazonForm() {
                if (!this.amazonValidateForm.marketPlaceId || !this.amazonValidateForm.shopName || !this.amazonValidateForm.sellerIdentifier || !this.amazonValidateForm.mwsToken) 
